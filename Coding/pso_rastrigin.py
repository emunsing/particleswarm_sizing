...
class Particle:
    def __init__(self, minx, maxx, seed):
        ...
        self.best_part_pos = np.array(dim)
...
def Solve(max_epochs, n, minx, maxx, initValues=None):
    ...
    if initValues is not None:
        [swarm[i].set_position(initValues[i],initCosts[i]) for i in range(initValues.shape[0])]
    
    ## Identify the best cost in the initial batch
    best_swarm_cost = sys.float_info.max # High initial value    
...
num_particles = 10
initial_successes = num_particles * 0.2
max_epochs = 100
...
# gridsearchSuccesses = pd.read_csv("../Results/gridSearchSuccesses_2016-07-08_10_07.csv", index_col=0)
# initPoints = gridsearchSuccesses.sort_values(by='cost')[0:initial_successes]
# initVariables = initPoints[['TEGparallel','TEGserial','batts','caps','SOC','V_b','V_c']].values
